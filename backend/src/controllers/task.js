...
const createTask = asyncHandler(async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      if (error.keyPattern && error.keyPattern.title === 1) {
        return res.status(400).json({
          success: false,
          message: 'You already have a task with this title. Please use a different title.',
          error: error.message
        });
      }
    }
...
const updateTask = asyncHandler(async (req, res) => {
  ...
  // Process assigned_to_users if it exists in the request (even if empty)
  if (updateData.hasOwnProperty('assigned_to_users')) {
    // Ensure it's an array (even if empty)
    if (Array.isArray(updateData.assigned_to_users)) {
      // Remove duplicates
      updateData.assigned_to_users = [...new Set(updateData.assigned_to_users)];
    } else {
      // If someone sends null or non-array, convert to empty array
      updateData.assigned_to_users = [];
    }
  }
  ...
  // Send notifications only for new assignees when there are assigned users
  if (updateData.hasOwnProperty('assigned_to_users') && 
      Array.isArray(updateData.assigned_to_users) && 
      updateData.assigned_to_users.length > 0) {
